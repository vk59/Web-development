        ...
        <div class="col-md-10 mx-auto col-lg-5">
          ...
          <div class="p-4 p-md-5 border rounded-3 bg-light">
            <form class="form-floating mb-4" id="formNickname">
              <input v-model="form.username" class="form-control" ref="floatingNickname" id="floatingNickname"
                     placeholder="nickname59">
            ...
            <!-- </form> -->
            <hr class="my-4">
            <small class="text-muted">If you don't have an account,
...
export default {
  ...
  methods: {
    async onSubmit(config) {
      try {
        const result = await this.axios.post('http://127.0.0.1:8000/auth/token/login/', {
          username: this.form.username,
          password: this.form.password
        })

        console.log('success', result)
        await this.$store.dispatch('SET_TOKEN', result.response.access)
        await this.$store.dispatch('SET_REFRESH_TOKEN', result.response.refresh)
        await this.$store.dispatch('SET_USERNAME', this.form.username)
